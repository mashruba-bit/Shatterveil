...
int coinFrames[COIN_FRAMECOUNT];
int coinAnimIndex = 0;

Lv3Coin coinsLv3[NUM_COINS_LV3];
int lv3CoinCount = 0;   // total collected
// ── Level Select Screen ───────────────────────────────────────
...
void iDraw() {
	...
	if (currentBg == 1)
		...
		for (int i = 0; i < NUM_COINS_LV3; i++) {
			if (!coinsLv3[i].collected) {
				int drawX = (int)coinsLv3[i].x - lv3CameraX;
				int drawY = (int)coinsLv3[i].y;
				iShowImage(drawX, drawY, COIN_W, COIN_H, coinFrames[coinAnimIndex]);
			}
			// "+1" pickup text — yellow, fades after COIN_TEXT_DURATION ticks
			if (coinsLv3[i].showText) {
				int textX = (int)coinsLv3[i].x - lv3CameraX + COIN_W / 2 - 8;
				int textY = (int)coinsLv3[i].y + COIN_H + 15;
				iSetColor(255, 220, 0);   // bright yellow
				iText(textX, textY, "+1", GLUT_BITMAP_HELVETICA_18);
			}
		}
...
void updateLv3Coins() {
	...
	// Advance animation
	static int coinTick = 0;
	if (++coinTick >= 5) {   // tweak 5 to change animation speed
		coinTick = 0;
		coinAnimIndex = (coinAnimIndex + 1) % COIN_FRAMECOUNT;
	}

	// Count down pickup text timers
	for (int i = 0; i < NUM_COINS_LV3; i++) {
		if (coinsLv3[i].showText) {
			coinsLv3[i].textTimer++;
			if (coinsLv3[i].textTimer >= COIN_TEXT_DURATION) {
				coinsLv3[i].showText = false;
				coinsLv3[i].textTimer = 0;
			}
		}
	...
	for (int i = 0; i < NUM_COINS_LV3; i++) {
		if (coinsLv3[i].collected) continue;

		// Coin hitbox (world coords)
		int cX1 = (int)coinsLv3[i].x;
		int cX2 = cX1 + COIN_W;
		int cY1 = (int)coinsLv3[i].y;
		int cY2 = cY1 + COIN_H;
		...
		bool overlapX = !(pX2 < cX1 || pX1 > cX2);
		bool overlapY = !(pY2 < cY1 || pY1 > cY2);

		if (overlapX && overlapY) {
			coinsLv3[i].collected = true;
			coinsLv3[i].showText = true;
			coinsLv3[i].textTimer = 0;
			lv3CoinCount++;
		}
	}
...
void initBackground() {
	...
	initFires();
	// ── Load Level 3 Coin frames ──────────────────────────────────
	char coinpath[128];
	for (int i = 0; i < COIN_FRAMECOUNT; i++) {
		sprintf_s(coinpath, "Images/coin_lv3/co%d.png", i + 1);
		coinFrames[i] = iLoadImage(coinpath);
	}

	// ── Place 5 coins in each of 6 bridge gaps ────────────────────
	// Helper: given gap start X and end X, place 5 coins centered in gap
	auto placeCoinRow = [&](int gapMinX, int gapMaxX, float gapY, int startIdx) {
		float gapCenter = (gapMinX + gapMaxX) / 2.0f;
		float totalW = 4.0f * COIN_SPACING;   // 5 coins = 4 spaces
		float startX = gapCenter - totalW / 2.0f;
		for (int c = 0; c < 5; c++) {
			coinsLv3[startIdx + c] = {
				startX + c * COIN_SPACING,
				gapY + COIN_Y_OFF,
				false, false, 0
			};
		}
	...
	// ── Bridge 9 coins (ON bridge9, before barrel2) — 4 coins ─────
	for (int c = 0; c < 4; c++) {
		coinsLv3[34 + c] = {
